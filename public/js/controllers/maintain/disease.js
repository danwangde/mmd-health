...
app.controller('addQuanListCtrl', ['$scope', '$modalInstance', 'items', '$modal', '$http', '$log', function ($scope, $modalInstance, items, $modal, $http, $log) {
    ...
    $scope.ce = function () {
        let getDis1 = '/curing/select_date_1?dis_id='+$scope.disInfo.disease_name;
        $http.get(getDis1).then((res) => {
...
app.controller('maintainBridgeDisease_controller', ['$scope', '$http','$filter', '$modal', '$log', function ($scope, $http,$filter, $modal, $log) {
    [$scope.diseaseInfo, $scope.totalItems, $scope.diseaseInfoNow] = [[], [], []];
    [$scope.currentPage, $scope.itemsPerPage, $scope.maxSize] = [1,12, 3];
    async function getData() {
        let gethome = '/curing/select_maintain?uid=' + $scope.app.globalInfo.userid;
