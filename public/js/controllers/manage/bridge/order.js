...
//轨迹
app.controller('sportCtrl', ['$scope','$http', '$modalInstance', 'items', '$modal', '$log', function ($scope, $http, $modalInstance, items, $modal, $log) {
    $scope.fullWidth = function () {
        var elem = document.getElementById("sport");
        requestFullScreen(elem);
    };
  setTimeout(map,500);
  async function map() {
      let url = '/curing/select_map?facilities_type='+items.facilities_type +'&facilitiesid='+items.facilitiesid + '&Reporting_time=' + items.Reporting_time;
      console.log(url);
      let res = await $http.get(url);
      console.log(res.data);
      $scope.list = res.data;
      //$scope.list = [{longitude:106.652024,latitude:26.617221},{longitude:106.652024,latitude:26.614221},{longitude:106.654024,latitude:26.612221},{longitude:106.657024,latitude:26.612221},{longitude:106.857024,latitude:26.812221}];
      var listLast = $scope.list.length-1;
      // 百度地图API功能
      var map = new BMap.Map("sport");    // 创建Map实例
      map.centerAndZoom(new BMap.Point($scope.list[0].longitude,$scope.list[0].latitude), 18);  // 初始化地图,设置中心点坐标和地图级别
      map.addControl(new BMap.MapTypeControl());   //添加地图类型控件

      map.enableScrollWheelZoom(true);     //开启鼠标滚轮缩放
      var carMk;
      var carMk2;
      var myBeginIcon = new BMap.Icon("../img/Mario.png", new BMap.Size(25,37), {imageOffset: new BMap.Size(0, 0)});//人
      var myEndIcon = new BMap.Icon("../img/Mario_meitu.png", new BMap.Size(25,37), {imageOffset: new BMap.Size(0, 0)});//人
      drawIcon();
      function drawGreenLine(i){
          var polyline = new BMap.Polyline([
              new BMap.Point( $scope.list[i].longitude,$scope.list[i].latitude),//起始点的经纬度
              new BMap.Point( $scope.list[i+1].longitude,$scope.list[i+1].latitude)//终点的经纬度
          ], {strokeColor:"#AD1AAC",//设置颜色
              strokeWeight:4, //宽度
              strokeOpacity:1});//透明度
      ...
      function drawIcon(){
          ...
          carMk2 = new BMap.Marker(
              new BMap.Point($scope.list[0].longitude,$scope.list[0].latitude),//起始点的经纬度
              {icon:myBeginIcon});
          map.addOverlay(carMk2);

          carMk = new BMap.Marker(
              new BMap.Point($scope.list[listLast].longitude,$scope.list[listLast].latitude),//终点的经纬度
              {icon:myEndIcon});
...
app.controller('manageBridgeOrder_controller', ['$scope', '$http', '$modal','$filter', '$log', function ($scope, $http, $modal,$filter, $log) {
    ...
    async function getData() {
        let gethome = '/curing/select_home_manage?uid=' + $scope.app.globalInfo.userid+'&power='+JSON.parse(sessionStorage.getItem('msg')).usertype;
    ...
    $scope.search = function () {
        $scope.diseaseInfoNow = $filter('order')($scope.diseaseInfo, $scope.oddNumbers, $scope.number);
    };
    $scope.sportLine = async function (item) {
