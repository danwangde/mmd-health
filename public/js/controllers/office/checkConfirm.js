'use strict';
app.controller('checkConfirm_controller', ['$scope', '$http', '$filter', '$modal', function ($scope, $http, $filter, $modal) {
...
app.controller('planCtrl', ['$scope', '$http', '$modal', '$log', 'items', '$modalInstance','$filter', function ($scope, $http, $modal, $log, items, $modalInstance, $filter) {
  console.log(items.item)
  $scope.status = items.item;
  $scope.ok = function () {
  ...
  $scope.search = function () {
    if($scope.data.type == '0') {
      $scope.data.type = '常规检测'
    }else {
      $scope.data.type = '荷载试验'
    }
    ...
    await $http.get(getSmallPlan).then((res) => {
      ...
      for(let item of $scope.smallData ){
        if(item.state !='ok') {
          ...
          $scope.state='已验收';
          let url = '/Testing/update_big_plan1?state='+$scope.state +'&id='+items.id;
