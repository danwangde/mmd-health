app.controller('checkTask_controller', ['$scope', '$http', '$filter', '$modal', function ($scope, $http, $filter, $modal) {
    console.log(111);
...
app.controller('addPlanCtrl', ['$scope', '$modalInstance', 'items', '$http', function ($scope, $modalInstance, items, $http) {
    console.log(items);
    $scope.bridgeTree =items.bridgeInfo;
    $scope.planData = items.item;
    //$scope.bridgeInfo = items.bridgeInfo;
    $scope.passageTree = items.passageInfo;
    ...
    function getBranchArr(data)
    {
        for (var i in data) {
            if (data[i].$$isChecked==true) {
                items.item.branch_arr.push(data[i].id);
            }
    ...
    function get2 () {
        if (items.item.facilities_type == '0') {
            $http.post('/Testing/insert_routine1', items.item).then((res1) => {
                console.log(res1.data)
            })
        } else {
            $http.post('/Testing/insert_load1', items.item).then((res1) => {
                console.log(res1.data)
            })
        }
    }
    ...
    $scope.savePlan = async function () {
        items.item.branch_arr = [];
        if(items.item.plan_type== 0) {
            getBranchArr($scope.bridgeTree);
        }else{
            getBranchArr($scope.passageTree);
        }
...
app.controller('planCtrl', ['$scope', '$http', '$modalInstance', '$modal', '$log', 'items','$filter', function ($scope, $http, $modalInstance, $modal, $log, items,$filter ) {
    ...
    $scope.search = function () {
        ...
        $scope.smallDataNow = $filter('taskCheck')($scope.smallData, $scope.data.type);
        console.log($scope.smallDataNow )
    }

    $scope.addPlan = async function (item) {
        console.log(item);

        let getBridge = '/Testing/select_bridge_struct?uid=' + JSON.parse(sessionStorage.getItem('msg')).userid + '&branch_id=' +JSON.parse(sessionStorage.getItem('msg')).branch_id;
        await $http.get(getBridge).then((res) => {
            $scope.bridgeInfo = res.data;
        });
        let getPassage = '/Testing/select_passage_struct?uid=' + JSON.parse(sessionStorage.getItem('msg')).userid + '&branch_id=' +JSON.parse(sessionStorage.getItem('msg')).branch_id;
    ...
    async function getPlanData() {
        ...
        await $http.get(getSmallPlan).then((res) => {
            ...
            $scope.totalItems =  $scope.smallData.length;
            getCurrentTimes();
    ...
    getPlanData();
       ...
       [$scope.smallData, $scope.totalItems, $scope.smallDataNow] = [[], [], []];
      ...
      function getCurrentTimes() {
          ...
          var start = ($scope.currentPage - 1) * $scope.itemsPerPage;
          $scope.smallDataNow = $scope.smallData.slice(start, start + $scope.itemsPerPage);
      }
