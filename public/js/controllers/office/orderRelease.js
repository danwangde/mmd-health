...
app.controller('manageBridgeOrder_controller', ['$scope', '$http', '$modal','$filter', '$log', function ($scope, $http, $modal,$filter, $log) {
  ...
  async function getData() {
    let gethome = '/curing/select_home?uid=' + $scope.app.globalInfo.userid+'&power='+JSON.parse(sessionStorage.getItem('msg')).usertype;
  ...
  $scope.changeStatus = async function () {
    if ($scope.searchData.oddStatus == '0') {
      let getcomfirms = '/curing/select_home?uid=' + $scope.app.globalInfo.userid + '&power=' + JSON.parse(sessionStorage.getItem('msg')).usertype;
      ...
      let getcomfirms = '/curing/select_order_ok?uid=' + $scope.app.globalInfo.userid + '&power=' + JSON.parse(sessionStorage.getItem('msg')).usertype;
      await $http.get(getcomfirms).then((res) => {
        res.data = res.data.map((item) => {
          item.StepID = step.get(item.StepID);
          return item
