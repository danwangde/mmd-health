...
app.controller('curing_controller',['$scope', '$modal', '$http', '$filter','$location', function ($scope,$modal, $http,$filter, $location) {


    [$scope.maintainInfo, $scope.totalItems, $scope.maintainNow]=[[],[],[]];
    [$scope.currentPage, $scope.itemsPerPage, $scope.maxSize] = [1,12,3];
    async function getData() {
        let gethome = '/bridgeinfo/select_manage_list?uid=' + $scope.app.globalInfo.userid+'&power='+JSON.parse(sessionStorage.getItem('msg')).usertype;
        ...
        $scope.$apply(() => {
            ...
            [...$scope.maintainNow] = $scope.maintainInfo;
            $scope.totalItems = $scope.maintainInfo.length;
            getCurrentTimes();
            for(let item of res.data){
                if(item.Reporting_time!=='0000-00-00 00:00:00') item.Reporting_time = get_date_str(new Date(item.Reporting_time));
                if(item.Completion_time!=='0000-00-00 00:00:00') item.Completion_time = get_date_str(new Date(item.Completion_time));
                if(item.send_time!=='0000-00-00 00:00:00') item.send_time = get_date_str(new Date(item.send_time));
    ...
    $scope.search = function () {
        $scope.maintainNow = $filter('curingstatis')($scope.maintainInfo, $scope.oddNumbers, $scope.facility);
    };
    ...
    function getCurrentTimes() {
        ...
        $scope.maintainNow = $scope.maintainInfo.slice(start, start + $scope.itemsPerPage);
    }
